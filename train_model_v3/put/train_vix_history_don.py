"""
Train FNO-DeepONet model on put options using VIX OHLC history (vix_history).
...
def get_config():
    ...
    return {
        ...
        "modes2": 2,
        "option_type": "put",
        # Fresh v5 dir: the old results_vix_history_don/ dirs were trained on VVIX
        # (mislabeled input) and are quarantined under _vix_is_vvix_LEGACY/ — never
        # write into them again.
        "results_dir": str(script_dir / "results_vix_history_don_v5"),
    }
...
EXPECTED_VIX_SHA256 = "e2f18c516c9d48fb1730c2c0203fce36900c70aa0635b99e64afdde157346301"


def add_provenance(cfg):
    """Validate the input HDF5 really is corrected-VIX v4, then record its provenance.

    This is a hard gate, not a label. The previous version hard-coded
    "source=CBOE SPX VIX" for whatever path it was handed, so a bare run (which
    defaulted to the v3 HDF5) would train on VVIX and stamp the output "VIX" --
    re-creating the exact mislabel this migration exists to undo. Provenance is
    now READ from the file's own phase1_* attrs, and any mismatch aborts before
    training starts. v3 files have no schema_version at all, so they fail here.
    """
    h5 = Path(cfg["h5_path"])
    if not h5.exists():
        raise SystemExit(
    ...
    schema, series = _s("schema_version"), _s("phase1_vix_source_series")
    sha = _s("phase1_vix_sha256").lower()

    if schema != "v4":
        raise SystemExit(
            f"REFUSING TO TRAIN: {h5.name} has schema_version={schema!r}, expected 'v4'.\n"
            "  v3 HDF5 files carry VVIX (vol-of-VIX) in `vix_history` -- the mislabeled\n"
            "  source CSV. Training on one and writing to a v5-named dir would silently\n"
            "  re-create the VVIX-labeled-as-VIX error. Rebuild with the _v4 preprocessing\n"
            "  scripts, or pass --h5-path to a genuine v4 file."
        )
    if "CBOE VIX" not in series:
        raise SystemExit(
            f"REFUSING TO TRAIN: {h5.name} vix source series is {series!r}, "
            "expected it to name 'CBOE VIX'."
        )
    if sha != EXPECTED_VIX_SHA256:
        raise SystemExit(
            f"REFUSING TO TRAIN: {h5.name} was built from a VIX CSV whose sha256 is\n"
            f"  {sha}\nbut the expected corrected-VIX CSV sha256 is\n"
            f"  {EXPECTED_VIX_SHA256}\n"
            "  The input data is not the reviewed CBOE VIX file."
        )
    ...
    cfg["vix_provenance"] = {
        ...
        "source_file": _s("phase1_vix_source_file"),
        "source_columns": _s("phase1_vix_source_columns"),
        "column_mapping": _s("phase1_vix_column_mapping"),
        "vix_csv_sha256": sha,
        "schema_version": schema,
        "t_basis": _s("phase1_t_basis"),
        "vix_raw_row_count": _s("phase1_vix_raw_row_count"),
        "vix_usable_row_count": _s("phase1_vix_usable_row_count"),
    }
    return cfg

def parse_args():
    parser = argparse.ArgumentParser(description="Train FNO-DeepONet on puts with vix history input (V3, analytical BS)")
    ...
    parser.add_argument("--h5-path", type=str, default=None,
                        help="Override the input HDF5 (default: deeponet_tensors_put_v5.h5). Must be a "
                             "dataset_version=v5 (quote-filtered) file built from the corrected CBOE VIX CSV; "
                             "a v4 (unfiltered) or v3 file is "
                             "refused at startup (v4 = unfiltered sample, v3 = VVIX). See _vix_is_vvix_LEGACY/DEFERRED_GPU_COMMANDS.md.")
    parser.add_argument("--results-dir", type=str, default=None,
                        help="Override the output directory (default: results_vix_history_don_v5). "
                             "Old results_vix_history_don/ dirs are VVIX-quarantined; do not reuse them.")
