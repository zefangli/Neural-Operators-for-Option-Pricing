...
All figures come from the independent CPU ablation evaluator in float32 at batch size 4096 over the
full test split.}
\label{tab:ablation}
\begin{tabular}{llrrr}
\toprule
...
float32 cancellation in $\log(\mathrm{clamp}(V/K, 10^{-8}))$ on the rows the scalar model prices at
the floor differs between device paths and precisions; the per-query models, which never reach the
floor, agree to $10^{-6}$ under every path and precision
...
\label{app:repro}
Table~\ref{tab:env} lists the direct package dependencies and their installed versions on the audit
and reproduction machine, captured on 2026-09-07 by \texttt{analysis/snapshot\_environment.py}
(\texttt{results/environment\_snapshot\_2026-09-07.txt}), which is the environment every evaluation
number reported in this paper was produced in. The August 2026 training environment was not
snapshotted and cannot be reconstructed from this table, so no version listed here should be read as
the version a model was trained under.

\begin{table}[H]
\centering
\small
\caption{Audit and reproduction environment, captured 2026-09-07, in which the evaluation numbers
reported in this paper were produced. It is not the training environment.}
\label{tab:env}
\begin{tabular}{ll}
\toprule
Component & Version \\
