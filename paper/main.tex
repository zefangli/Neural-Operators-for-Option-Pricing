...
\begin{abstract}
The Black--Scholes formula is exact once the implied volatility is known. However, most neural option
pricers discard this structure and learn an opaque map straight to price. Here, we learn a
\emph{per-contract implied volatility} $\hat\sigma$ and feed it through the closed-form
...
results follow. Per-query conditioning has the largest effect: one scalar $\hat\sigma$ per market
state rather than per contract collapses put $R^2(\log)$ from 0.978 to $-0.317$ and
...
surveyed by~\citet{RufWang2020Review}. This throws away structure that is already known in closed
form: under the standard Black--Scholes assumptions, the price is an explicit, differentiable function
of one hard-to-observe quantity, the implied volatility $\sigma$, while everything else in the formula
(moneyness, maturity, rate, dividend yield) is directly observed. However, in the architectures that
literature covers, the learned approximator sits on the pricing map itself, so the analytical formula
is either replaced or numerically reproduced rather than retained as an exact output layer, and the
one genuinely unobserved quantity in it is never isolated as the network's sole target. Here, we train
...
quotations), but it is not zero. (b) The three encoders are \emph{not} parameter-matched---because
the flattened FNO output is projected by a single dense layer whose input width scales with the grid,
that projection alone holds 766{,}080 parameters for the $11{\times}17$ surface grid, 430{,}208 for
...
The flattened FNO output is projected to the latent vector by a single dense layer whose input width is
proportional to the grid size, so the three encoders differ in parameter count: that projection alone
holds 766{,}080 parameters for the surface grid, 430{,}208 for SPX history, and 344{,}192 for VIX
history. The branch comparison in Section~\ref{sec:results-branch} is consequently not a
...
$10^{-6}$ in a vanishing band around the money. No contract in this dataset reaches that regime.
...
variance estimate for them. Test-set metrics reported throughout are computed on the normalized price
$V/K$, not raw dollar prices, and are averaged over the full test split (no subsampling) by a held-out
evaluation script loaded from each run's best checkpoint by validation loss.
...
and fits no parameter at all; a single constant $\hat\sigma$ per option type (flat-vol BS), fitted on
the training split under two separate objectives, minimum MSE on $\log(V/K)$ and minimum MSE on $V/K$,
...
surface, then BS). All three are evaluated on the same filtered test split as the models. The same
sweep also evaluates a family of VIX-level baselines that set $\hat\sigma$ to the day's VIX level times
a single train-fitted scale factor; we do not tabulate these as headline comparators, since they use a
market observable that is not one of the three branch inputs, but we quote the price-space-fitted
variant, \texttt{vix\_level\_scaled\_pxfit}, in Section~\ref{sec:results-branch}, where it bears on how
the VIX result should be read.
...
not parameter-matched (766{,}080 / 430{,}208 / 344{,}192 parameters in the flattening projection
alone), and they also differ in grid size, spectral modes, and input normalization, so input content,
timing, and capacity vary together and what is ranked is three complete configurations rather than
three raw inputs. Third, the VIX-history tensor is divided by the window's final VIX close, which
removes the absolute level of VIX and leaves only the shape of the 21-day path; the correct reading of
the VIX result is that \emph{this close-normalized representation} underperforms the surface, not that
VIX history is uninformative in general. The \texttt{vix\_level\_scaled\_pxfit} baseline, which
uses the raw VIX \emph{level} scaled by a single train-fitted constant, attains the highest call-side
$R^2(\log)$ of any non-neural baseline we evaluated, 0.7904 (price-space $R^2 = 0.999809$), above the
0.7642 of spline interpolation of the observed surface in Table~\ref{tab:baselines-call}.
...
canonical Table~\ref{tab:t1} values; the separate ablation evaluator reproduces them to the fifth
decimal, returning 0.221896, 0.977899, and 0.294127 for the three cells where the two scripts differ.}
\label{tab:ablation}
\begin{tabular}{llrrr}
\toprule
...
0.978 to $-0.317$. On puts, the price-space effect is larger still: $R^2(\text{price})$ falls from
...
separates them sharply. Both constant-$\hat\sigma$ rows are fitted on the training split, one by
...
constant-$\hat\sigma$ rows are fitted on the training split, one by minimizing MSE on $\log(V/K)$ and
...
the best baseline on price-space. On puts, the two metrics disagree on which pricer wins: IV-surface
spline interpolation edges out the model on price-space (0.996594 vs.\ 0.993780), while the model
leads on log-space by 3.50 in $R^2$ (0.981298 vs.\ $-2.518559$). Whichever pricer a reader considers
``better'' for puts depends on which metric they weight; we report both rather than only the more
flattering one.
...
bucket, 50{,}000-row uniform sample of the test split, IV-surface architecture-A/B models.}
\label{fig:f5}
\end{figure}
...
\paragraph{Loss function.} We use plain MSE on $\log(V/K)$ throughout; an informal comparison against a
Huber loss did not improve the headline log-space metric and is not reported as a controlled ablation.
...
matters: removing it collapses put $R^2(\log)$ from 0.978 to $-0.317$ and put $R^2(\text{price})$ to
...
\label{app:repro}
All experiments were run under a single conda environment. Table~\ref{tab:env} lists the direct package
dependencies and their installed versions, obtained by querying the environment directly.
...
Each run's exact configuration (seed, hyperparameters, dataset version and checksum) is saved alongside
its checkpoints at training time.
