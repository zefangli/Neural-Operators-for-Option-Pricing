"""
Train FNO + MLP model on call options using implied volatility surface (branch_u).
V2: Two-stage training with pretrained, frozen Network 2.
...
Architecture:
  ...
  Network 2: MLP([log_moneyness, T_years, r, q, sigma_hat]) -> log(V/K)
            pretrained on BS analytical formula, frozen during Stage 2.

Losses:
  - Data MSE on log(V/K) only
  - PDE, arbitrage, and BS anchor losses removed (Network 2 is near-exact BS oracle)
"""
...
class MLP_Predictor(nn.Module):
    """MLP taking [log_moneyness, T_years, r, q, sigma_hat] -> unconstrained log(V/K).
...
def train_model(config):
    ...
    pretrained_net2_path = config["pretrained_net2_path"]
    ...
    # Load pretrained Network 2 and freeze it
    print(f"Loading pretrained Network 2 from: {pretrained_net2_path}")
    net2_state = torch.load(pretrained_net2_path, map_location=device)
    model.net2.load_state_dict(net2_state)
    model.freeze_net2()

    net1_params = sum(p.numel() for p in model.net1.parameters())
    net2_params = sum(p.numel() for p in model.net2.parameters())
    print(f"Network 1 (trainable): {net1_params:,} params")
    print(f"Network 2 (frozen):    {net2_params:,} params")

    # Optimizer — Network 1 only
    ...
    # ============ PHASE 1: ADAM ============
    print("\n" + "=" * 60)
    print("PHASE 1: Adam Optimizer (Network 1 only, Network 2 frozen)")
    print("=" * 60)
    ...
    for epoch in range(epochs_adam):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.net1.parameters(), grad_clip)
            optimizer_adam.step()
    ...
    # ============ PHASE 2: FINE-TUNE ============
    print("\n" + "=" * 60)
    print("PHASE 2: Fine-tuning with Adam (Low LR, Network 1 only)")
    print("=" * 60)

    optimizer_ft = optim.Adam(model.net1.parameters(), lr=finetune_lr, weight_decay=1e-6)
    epochs_no_improve = 0
...
def get_config():
    ...
    return {
        ...
        "modes2": 8,
        "option_type": "call",
        "pretrained_net2_path": str(script_dir / "pretrained_net2" / "net2_pretrained.pth"),
        "results_dir": str(script_dir / "results_vol_surface"),
    }


def parse_args():
    parser = argparse.ArgumentParser(description="Train FNO+MLP on calls with vol surface input (V2)")
    ...
    parser.add_argument("--pretrained-net2", type=str, default=None)
...
if __name__ == "__main__":
    ...
    if args.seed is not None:
        cfg["seed"] = args.seed
    if args.pretrained_net2 is not None:
        cfg["pretrained_net2_path"] = args.pretrained_net2
