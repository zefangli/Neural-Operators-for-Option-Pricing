"""
Train FNO + MLP model on call options using SPX OHLCV history (spot_history).
...
Architecture:
  ...
  Network 2: MLP([log_moneyness, T_years, r, q, sigma_hat]) -> log(V/K)  [frozen]

Losses: Data MSE only
"""
...
def train_model(config):
    ...
    # ============ PHASE 1: ADAM ============
    print("\n" + "=" * 60)
    print("PHASE 1: Adam Optimizer (Network 1 Only)")
    print("=" * 60)
    ...
    for epoch in range(epochs_adam):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loss, loss_dict = compute_loss(
                model, branch, log_m, T, r, q, target,
            )
            loss.backward()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train FNO+MLP on calls with spot history input (V2)")
