...
Losses:
  ...
  - Option-type-specific arbitrage constraints (positive delta, positive gamma for calls)
...
def compute_loss(model, branch, log_moneyness, T_years, r, q, target_v_log,
    ...
    # 1. Data loss on log(V/K)
    loss_data = F.mse_loss(log_v_hat, target_v_log)

    # 2. PDE loss in normalized price space
    ...
    if option_type == "call":
        # Calls: positive delta (dv/dM > 0), positive gamma (d2v/dM2 > 0)
        loss_arb = torch.mean(F.relu(-dv_dM)) + torch.mean(F.relu(-d2v_dM2))
    else:
        # Puts: negative delta (dv/dM < 0), positive gamma (d2v/dM2 > 0)
        loss_arb = torch.mean(F.relu(dv_dM)) + torch.mean(F.relu(-d2v_dM2))

    # 4. BS anchor loss (Network 2 only, synthetic samples)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train FNO+MLP on calls with vol surface input")
